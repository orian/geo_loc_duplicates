...
DEFINE_bool(use_unique_id, false, "Use unique_id CSV column to verify accuracy.");
...
bool IsOrig(const PointData& data) {
  return data.id_ == data.unique_id_;
}
...
#define LOG_IF(condition) if (FLAGS_print_debug && (condition)) std::cout
#define LOG if (FLAGS_print_debug) std::cout

int true_positives = 0;
int duplicate_counter = 0;

void FindCloseByLongitude(float radius, const PointData& active_start,
    const trilib::RBTree<PointData*, PointerCmp<PointData, PointOrderByLong>>& active) {
  ...
  auto iter_first = active.LowerBound(&by_long_start); // iter_first should be always non-empty because of a node itself
  ...
  while (iter_first != iter_last) { // wrong condition
    ...
    bool dist_close_enough = !is_self && Distance(active_start, **iter_first) < radius;
    if (dist_close_enough) {
      LOG << "\t\tconsidering duplicate: " << **iter_first
                << std::endl;
      ++duplicate_counter;
      if (FLAGS_use_unique_id && (IsOrig(active_start) || IsOrig(**iter_first)) && active_start.unique_id_ == (*iter_first)->unique_id_) {
        ++true_positives;
      }
    }
    else if (is_self) {
      ...
      LOG << "\t\tskipping: " << **iter_first << Distance(active_start, **iter_first) << std::endl;
    }
    ++iter_first;
  }
  if (long_size > 0) {
...
// taking addres of items in vector sounds like a bad idea (if vector reallocates -> huge problems)
void Cluster(float radius, vector<PointData>* points) {
  ...
  // In tree we will keep all points closer than radius to active point, but this time sorted by longitude.
  ...
  while (active_start != points->end() - 1) {
    LOG_IF(active_end != points->end()) <<"* " << *active_end << "  " << DistanceLat(*active_start, *active_end);
    if (active_start != active_end && (active_end == points->end() ||
        DistanceLat(*active_start, *active_end) >= radius)) {
      // find all close
      LOG <<" waiting \n\nactive for searching duplicates: " << *active_start << std::endl;
